...
def has_sin?(string)
  true if string.scan(/\d{3}-\d{3}-\d{3}/) != ""
end
...
def format_sins(string)
  capture = string.scan(/\d{3}[^\w]?\d{3}[^\w]?\d{3}/)
  if capture.empty?
    return string
  end 
  final_array = []
  capture.each do |sin|
    if sin.length == 9
      first_set = sin.slice(0..2)
      mid_set = sin.slice(3,3)
      last_set = sin.slice(sin.length - 3, 3)
      final_array << (first_set + "-" + mid_set + "-" + last_set)
    else
      final_array << sin.gsub(/[^\w]/, "-")
    end
  end
  p final_array.join(", ")
end
